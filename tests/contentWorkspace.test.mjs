...
test('a click on a project-content card passes the entry.id as a focus target, and ContentINKAScreen applies it', () => {
  ...
  // Личном кабинете, ContentPanel hand-off) are untouched — they still only
  // flip the screen. Настройки passes it as part of the settings prop bag
  // now (embedded tab in MasterDashboardScreen), not a standalone JSX
  // attribute.
