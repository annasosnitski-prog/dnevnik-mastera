...
меняется.** Все ссылки на строки — по состоянию `main` (сверено: ветка аудита
основана на текущем tip `main`, файл `TattoDiary.tsx` — 13 672 строки, с
момента аудита не менялся, номера строк актуальны).
...
**Важно: `ClientNote` не содержит поля срока** — только `urgency`, `done`,
`createdDate`, `projectId`, `photos`, `text` (см. интерфейс, 215–223). Даты
«к какому числу» у задачи нет вообще.
...
скрытой (менее вероятный сценарий).

**Спецификация исправленного ключа (уточнено).** Одного
`sourceType + sourceId + rule + dueDate` недостаточно: у проекта «следующий
шаг» — это перезаписываемое поле на той же сущности (`nextActionText`/
`nextActionDate`), поэтому мастер может задать **новое** действие с **тем же**
сроком (`dueDate` не изменился) — ключ совпадёт со старым, скрытым, и новое
действие снова не покажется. Ключ обязан меняться при смене самого действия, а
не только его даты. Варианты (выбор — в PR 4):
- **`nextActionId`** — при каждой правке `nextActionText`/`nextActionType`/
  `nextActionDate` проекту присваивается новый id действия; ключ включает его.
  Чище всего, но требует поля на `Project` и его нормализации.
- **`actionRevision`** — числовой счётчик, инкремент при любой смене действия;
  ключ включает ревизию. Дешевле, без нового «идентификатора».
- **`actionSignature`** — детерминированный хэш от
  `(nextActionText + nextActionType + nextActionDate)`; ключ включает подпись.
  Без хранения нового поля, но «пустой→снова пустой» текст даст ту же подпись
  (для project_action_due не проблема — при пустом действии правила нет).
Итоговый ключ: `createReminderKey({sourceType, sourceId, rule, dueDate,
actionSignature})` (или `nextActionId`/`actionRevision` вместо
`actionSignature`). Любая правка действия → новый экземпляр напоминания →
прежнее «скрыто» больше не применяется.
**В текущем PR не чиню (аудит), фиксирую как обязательное к решению в PR 4.**
...
1. **`projectReminderKey` нестабилен к смене действия** (см. вопрос 8) —
   ...
   ключ с `rule` + `dueDate` + `actionSignature`/`nextActionId`/
   `actionRevision`).
2. **У `ClientNote` (Task) нет `dueDate`.** Поэтому «напоминания из Task»
   (в плане — PR 6) **нельзя** сделать простым подключением существующих задач:
   у правил `task_due`/`task_overdue` нет даты, к которой их привязать. До PR 6
   требуется отдельное решение (мини-PR, реализовать **не сейчас**):
   - добавить `dueDate: string | null` в `ClientNote`;
   - обновить нормализацию (`normalizeClient` notes-блок ~955, и master-notes
     нормализацию ~1957) — дефолт `null`;
   - добавить поле срока в форму заметки (`NoteItem` edit-блок);
   - проверить экспорт/импорт (`handleExport`/`replaceAllData`; master-notes
     идут через localStorage — см. п. 4);
   - старым задачам оставить `dueDate: null` (без даты → не показывать
     автоматически, как и задумано правилами).
3. **Задачи мастера (`masterInfo.notes`) живут в localStorage, а не в
...
4. **Задачи мастера не попадают в бэкап через `handleExport`** — он кладёт
   clients/projects/contentEntries, а `masterInfo` (с задачами мастера) лежит
   в localStorage отдельно и в основной экспорт не входит. Риск потери данных
   при переносе устройства/переустановке. Отдельный **safety-PR** (см. очередь):
   либо включить `masterInfo.notes` (или `masterInfo` целиком) в полный бэкап,
   либо сделать отдельный экспорт `masterInfo`. Не в рамках рефакторинга.
...
- **PR 5.5 (обязателен ПЕРЕД PR 6) — `dueDate` на Task.** Отдельный маленький
  PR (модель + данные, не UI-логика напоминаний): добавить
  `ClientNote.dueDate: string | null`, обновить обе нормализации (дефолт
  `null`), поле срока в форму заметки, проверить экспорт/импорт, старым — `null`.
  Без этого PR 6 невозможен (у Task нет даты).
- **PR 6 — напоминания из Task.** Только после PR 5.5. Первый источник — Task
  (правила due/overdue, done/snooze/без даты → не показывать). Действия карточки:
  Выполнить/Отложить/Открыть проект (или Открыть Планнер, если у задачи нет
  `projectId`). Новых задач автоматически не создавать.
...
### Отдельные небольшие PR (вне основной цепочки рефакторинга)

- **PR (модель) — `NextActionType`.** Ввести
  `type NextActionType = 'contact_client' | 'collect_information' |
  'prepare_design' | 'schedule_consultation' | 'schedule_session' |
  'receive_deposit' | 'prepare_session' | 'check_healing' |
  'schedule_next_session' | 'review_project' | 'other'` и поле
  `Project.nextActionType: NextActionType | null`. Нормализация старых проектов
  (`normalizeProject` ~970) → `nextActionType: null`. **Без** распознавания типа
  по старому `nextActionText`. Не создавать Task из nextAction автоматически.
  Может пойти до/после слоя напоминаний — самостоятелен.
- **Safety-PR — бэкап задач мастера.** `masterInfo.notes` сейчас в localStorage
  и не входит в `handleExport`. Включить `masterInfo` (или его `notes`) в полный
  бэкап, либо сделать отдельный экспорт `masterInfo`. Приоритет — до накопления
  большого объёма задач мастера (риск потери).
