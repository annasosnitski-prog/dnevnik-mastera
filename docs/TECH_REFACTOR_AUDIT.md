...
## 6. Обновление: ленивая загрузка экранов (жалоба на подвисание при старте)

Отчёт: старт приложения ощутимо подвисает (~1.5 с). Причина — не сама
IndexedDB (она асинхронна и не блокирует поток), а сборка: до этого
изменения `vite build` выдавал единственный бандл 534 КБ/147 КБ gzip
(предупреждение Vite про >500 КБ), потому что все экраны, вынесенные по
плану выше в отдельные файлы (раздел 5), всё равно импортировались в
`TattoDiary.tsx` статически — устройство разбирало и выполняло код всех
дашбордов и обеих мини-игр ещё до отрисовки самого первого экрана
(`screen === 'list'`).

Экраны и мини-игры, у которых не было статических зависимостей в обратную
сторону (обратно в `TattoDiary.tsx`), переведены на `React.lazy` +
`Suspense`: `SummaryScreen`, `AdminDashboardScreen`, `WorkshopScreen`,
`SettingsScreen`, `DetailScreen`, `RPSGame`/`RPSTauntFace`, `CupsGame`,
`BlackjackGame`. `AddChatLinkForm`/`AddMasterLinkForm` пришлось сначала
перенести из `screens/DetailScreen.tsx` в `client/ClientControls.tsx` —
иначе статический импорт этой пары в `TattoDiary.tsx` утягивал весь
DetailScreen (2600+ строк, самый большой экран) обратно в основной бандл,
сводя ленивую загрузку на нет. Итог по `vite build`: основной чанк
534.75 КБ/147.36 КБ gzip → 418.61 КБ/120.59 КБ gzip, предупреждение об
избыточном чанке пропало.

`MasterDashboardScreen` и `ContentINKAScreen` (674 и ~1190 строк) всё ещё
определены прямо в `TattoDiary.tsx` (раздел 2, «Высокий риск» — последние
пункты плана из раздела 5) и поэтому не могли быть переведены на lazy без
их вреза в отдельный модуль в рамках этого же PR. Следующая по значимости
цель для дальнейшего снижения стартового бандла — именно они.
