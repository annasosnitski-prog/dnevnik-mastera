...
// Дневник из-за этого ничего не отменяет — при открытии синк и так не
// запускается, — но запись в журнал уходит: это единственное доказательство,
// что устройство не вытягивает полный прогон.
//
...
export function useSyncDriver(
  ...
  const pairWithCode = useCallback(async (code: string) => {
    ...
    if (result.ok) {
      setLastError(null);
      setPhase('paired');
      // Синк сразу после привязки — единственный автоматический прогон,
      // который остался, и это не «синк при открытии»: мастер только что
      // сама нажала «Привязать». Устройство подключили к облаку именно
      // затем, чтобы увидеть на нём свои данные; заставлять её после этого
      // искать вторую кнопку (или ждать шесть часов) было бы издевательством.
      void runSync(true);
      return { ok: true };
    }
    const message =
